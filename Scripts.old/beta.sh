...
# Définition des chemins | Defining the paths
#libfncts="../lib/functions"
#libvars="../lib/variables"


# Inclusion des fichiers source des fonctions | Including the functions source files
for _ in "../lib/functions/*.sh"; do
    # shellcheck source="../lib/functions/*"
    source _ || echo "$_[@] : not found" && exit 1
done



# Inclusion des fichiers source des variables | Including the variables source files
# shellcheck source=../lib/variables/args.sh
#source "$libvars/args.sh"        || echo "$libvars/args.sh : Not found"        && exit 1
# shellcheck source=../lib/variables/colors.sh
#source "$libvars/colors.sh"      || echo "$libvars/colors.sh : Not found"      && exit 1
# shellcheck source=../lib/variables/date.sh
#source "$libvars/date.sh"        || echo "$libvars/date.sh : Not found"        && exit 1
# shellcheck source=../lib/variables/filesystem.sh
#source "$libvars/filesystem.sh"  || echo "$libvars/filesystem.sh : Not found"  && exit 1
# shellcheck source=../lib/variables/text.sh
#source "$libvars/text.sh"       || echo "$libvars/text.sh : Not found"         && exit 1

# Inclusion des fichiers source des fonctions | Including the functions source files
# shellcheck source="../lib/functions/Echo.sh"
#source "$libfncts/Echo.sh"       || echo "$libfncts/Echo.sh : Not found"       && exit 1
# shellcheck source="../lib/functions/Filesystem.sh"
#source "$libfncts/Filesystem.sh" || echo "$libfncts/Filesystem.sh : Not found" && exit 1
# shellcheck source="../lib/functions/Headers.sh"
#source "$libfncts/Headers.sh"    || echo "$libfncts/Headers.sh : Not found"    && exit 1
# shellcheck source="../lib/functions/Install.sh"
#source "$libfncts/Install.sh"    || echo "$libfncts/Install.sh : Not found"    && exit 1


## VERSION
VER_SCRIPT="2.0"    # Version actuelle du script.
VER_PACKS=""        # Version d'installation (SIO ou personnelle)
